...
class KneserNeyLM:
    ...
    def calculate_kneser_ney_sentence_probability(self, sentence, normalize_probability=False):
        mixgram_sentence_probability_log_sum = 0
        trigrams = ngrams(sentence, 3, pad_left=True, pad_right=True, left_pad_symbol='<s>',
                          right_pad_symbol='</s>')
        for trigram in trigrams:
            log_prob = self.logprob(trigram)
            if log_prob is not None:
                mixgram_sentence_probability_log_sum += log_prob
        return math.pow(2, mixgram_sentence_probability_log_sum) if normalize_probability \
            else mixgram_sentence_probability_log_sum

    def calculate_number_of_ngrams(self, sentences):
        ngram_count = 0
        for sentence in sentences:
            ngram_count += len(sentence) + 2
        return ngram_count

    def calculate_kneser_ney_perplexity(self, sentences):
        number_of_ngrams = self.calculate_number_of_ngrams(sentences)
        average_perplexity = 0
        knesey_ney_sentence_probability_log_sum = 0
        for sentence in sentences:
            knesey_ney_sentence_probability_log_sum -= self.calculate_kneser_ney_sentence_probability(sentence)
        return math.pow(2, knesey_ney_sentence_probability_log_sum / number_of_ngrams)